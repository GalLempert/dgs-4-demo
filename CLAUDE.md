...
A demo GraphQL stack built with Netflix DGS 4.9.x / graphql-java 17 on Spring Boot 2.4.2, Spring Data JPA / Hibernate 5.4, Java 11. Multi-module Maven project: a reusable, domain-agnostic GraphQL infrastructure (the framework — the shared "how") and two completely separate runnable services built on it (person-service :8080, company-service :8081 — each its own process, database, schema and API; the unique "what"). Cross-service links are id-only reference stubs, federation-ready (`docs/FEDERATION.md`). Beside the full framework lives the lite track (`docs/LITE.md`): `graphql-infrastructure-lite` (a four-class, essentials-only alternative infrastructure - schema load, /graphql, dispatch to resolver beans; no JPA/validation/filtering/errors) reused by two tiny standalone demo services, person-service-lite :8082 and company-service-lite :8083. The two tracks share no code.
...
Detailed docs exist and should be consulted before structural changes: `docs/ARCHITECTURE.md` (module split, startup wiring, life of a request), `docs/EXTENDING.md` (step-by-step recipes for adding fields, filters, predicates, error codes, whole domains), `docs/FILTERING.md`, `docs/FILTER-COMPOSITION.md` (designed but NOT yet implemented), `docs/MUTATIONS.md` (the five standard framework mutations: save-new, update-by-filter, save-or-update, save-or-override, delete-by-filter — write pipeline, hooks, wiring), `docs/REPLICATION.md` (sequence-based replication feed: `personsBySequence`, soft deletes, filtered replication), `docs/REPLICATION-FILTERING.md` (how the feed's dynamic filter is evaluated — page by sequence, re-check ids, partition — and why filtered replication stays correct with a stateless server), `docs/FEDERATION.md` (separate services, cross-service id-only references, later-PR federation plan), `docs/API-WALKTHROUGH.md` (screenshot-guided playground tour of the four standard queries), `docs/DAL-ALTERNATIVES.md`, `docs/UPGRADE-PERFORMANCE.md`, `docs/MIGRATION-FROM-GRAPHQL-JAVA-ANNOTATIONS.md` (porting a code-first graphql-java-annotations service onto this framework; the transitional `DataFetcherAdapters` bridge lives in the infrastructure `graphql.migration` package)), `docs/LITE.md` (the lite track: the minimal `graphql-infrastructure-lite` module, its two demo consumers, and the mapping from each deliberate omission to its full-track counterpart).
...
- **Every resource is a replicated resource**: entities extending `BaseEntity` get the whole replication stack (see `docs/REPLICATION.md`) from the infrastructure `replication` package: `ResourceRepository` → `ResourceDal` → `ResourceService` → `ReplicationResolverFactory`/`MutationResolverFactory`. A domain wires them with empty/near-empty subclasses plus one `@Bean` per standard query/mutation field (see `docs/MUTATIONS.md` for the five standard mutations and their service-layer hooks). Every write stamps a fresh per-table sequence; deletes are soft (`softDelete`) and regular reads exclude deleted rows.
