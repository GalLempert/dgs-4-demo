...
- **Technical-truth hierarchy**: the technical fields every resource has (id, optimistic-locking `version`, createdAt, updatedAt, and for replicated resources sequence + deleted) are declared once, in one aligned hierarchy per representation — entities: `BaseEntity` → `ReplicatedEntity`; view DTOs: `ResourceView` → `ReplicatedResourceView`; GraphQL schema: `interface Resource` → `interface ReplicatedResource` (in `common.graphqls`; every resource type declares `implements ReplicatedResource & Resource`). `GraphQLModelTypeResolver` resolves concrete types for interface-typed fields via `@GraphQLModel`. Domain types never redeclare these fields in Java — only in their schema type (GraphQL requires implementing types to repeat interface fields).
